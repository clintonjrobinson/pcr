...
class UIPackage {
  ...
  toString(filter, forBundle) {
    ...
    if (IS_PRODUCTION && !forBundle && filter !== 'ui') {
      switch (filter) {
        case 'css':
          return `<link rel="stylesheet" type="text/css" href="/pcr/ui/${this.name}/bundle/css">\n`;
        case 'js':
          return `<script src="/pcr/ui/${this.name}/bundle/js"></script>\n`;
        case 'msx':
          return `<script src="/pcr/ui/${this.name}/bundle/msx"></script>\n`;
        case 'polymer':
          return `<link rel="import" href="/pcr/ui/${this.name}/bundle/polymer">`;
      }
    }
    ...
    for (let file of this.contents) {
      ...
      switch (filter) {
        ...
        case 'js':
          match = (file.constructor === JavaScriptFile || file.constructor === MithrilFile);
          break;
