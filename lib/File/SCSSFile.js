...
class SCSSFile extends File {
  ...
  get development() {
    if (!this._development) {
      this._development = `<link rel="stylesheet" type="text/css" href="/pcr/ui/${this.ui.name}/${this.name}" shim-shadowdom>\n`;
    }
  ...
  get production() {
    if (!this._production) {
      this._production = '<style shim-shadowdom>' + cleanCss.minify(this.transformed).styles + '</style>';
    }
